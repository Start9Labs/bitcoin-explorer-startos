...
This repo packages [BTC RPC Explorer](https://github.com/janoside/btc-rpc-explorer) for StartOS.
...
```bash
git submodule update --init --recursive    # initialize the explorer submodule
npm ci                                     # install dependencies
make                                       # build the universal .s9pk
```

The `explorer` image is built from the `explorer/` git submodule — it must be checked out before `make`.

## Updating the upstream version

BTC RPC Explorer is built from the `explorer/` git submodule. There is no `dockerTag` in the manifest.
